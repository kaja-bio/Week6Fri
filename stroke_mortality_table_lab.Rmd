...
```{r}
joined_data <- full_join(hb, stroke_mortality_raw, by = join_by(HB ==HBR))
```
...
```{r}
first_table <- joined_data %>% 
  filter(Year == 2018, HBName %in% c("NHS Lothian", "NHS Greater Glasgow and Clyde"), AgeGroup == "75plus years", Sex != "All") %>% 
...
```{r}
gt_table <- first_table %>% 
  summary_rows(columns = c(NumberOfDeaths, CrudeRate, EASR),
               fns = list(label = "Average", fn = "mean"), 
                          fmt = ~ fmt_number(., decimals = 2)) %>% 
  grand_summary_rows(columns = c(NumberOfDeaths, CrudeRate, EASR),
               fns = list(label = "Overall Average", fn = "mean"), 
               fmt = ~ fmt_number(., decimals = 2)) %>% 
  fmt_number(decimals=2) %>% 
  tab_header(title="Stroke Mortality in 2018 in Adults Aged 75+", subtitle="Data from NHS Lothian and NHS Greater Glasgow and Clyde") %>% 
